...
function modifier_a_scroll:OnCreated(args)
	...
	if IsServer() then
		CustomNetTables:SetTableValue("sync","a_scroll_buff", { magic_resist = self.MagicResistance,
																armor = self.Armor })
	end
end
...
function modifier_a_scroll:GetModifierMagicalResistanceBonus()
    ...
    elseif IsClient() then
        local magic_resist = CustomNetTables:GetTableValue("sync","a_scroll_buff").magic_resist
        return magic_resist 
    end
end
...
function modifier_a_scroll:GetModifierPhysicalArmorBonus()
	if IsServer() then        
    	return self.Armor
    elseif IsClient() then
        local armor = CustomNetTables:GetTableValue("sync","a_scroll_buff").armor
        return armor 
    end
end
