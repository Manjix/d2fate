...
function OnBerserkStart(keys)
  ...
  --   	ParticleManager:SetParticleControl(particle, 1, caster:GetAbsOrigin() )
  		...
  		if caster:HasModifier("modifier_madmans_roar_silence") then
  			caster:SetHealth(math.min(hplock * 2, caster:GetMaxHealth() - caster:GetModifierStackCount("modifier_gae_buidhe", keys.ability) * 10))
		else caster:SetHealth(math.min(hplock, caster:GetMaxHealth() - caster:GetModifierStackCount("modifier_gae_buidhe", keys.ability) * 10))
			end
